...
> 阅读此引导之前，我们推荐先看看 <Link text="React 教程" href="/react-tour" /> 或 <Link text="Vue 教程" href="/vue-tour" />。
> 教程里解释了什么是 `/renderer/_default.page.*` 文件

{/* ## `onBeforeRender()` */}

获取数据常用的方法是使用 `onBeforeRender()` 钩子函数
...
为了使用更加方便，我们可以使用<Link text="Custom Export/Hook" href="/exports" />，然后在 `/renderer/_default.page.server.js` 定义一次 `onBeforeRender()`，而不必在每个页面都定义一个 `onBeforeRender()` 钩子
...
使用 GraphQL 时，我们在组件级别定义 GraphQL queries/fragments，同时在单独全局的 `onBeforeRender()` 钩子中获取 GraphQL 数据
