...
使用预渲染的话，我们不需要 Node.js 服务：我们的应用程序仅包含静态文件（HTML、JS、CSS、images 等），我们可以将其部署到“静态主机”上，比如： <Link href="/github-pages" text="GitHub Pages" />， [Cloudflare Pages](/cloudflare-pages)， 或 [Netlify](/netlify)
...
对于具有参数化路由的页面（例如：`/movie/@movieId`），我们必须使用 [`prerender()` 钩子](/prerender) 指定需要预渲染的 URL 列表。`prerender()` 钩子也可用于加速预渲染
